//! Внутренняя системная библиотека surface protocol.
//!
//! Ею пользуются `uid`, `renderd`, оконный сервер и диагностические системные
//! процессы. Обычное приложение подключает публичные Window/SystemUI/Canvas
//! DLL и никогда не видит `SurfaceId`, `GraphicsBuffer` или timeline. Библиотека
//! скрывает capability IPC, создаёт приватный event endpoint и превращает
//! wire-сообщения compositord в типизированные события. Пиксели никогда не
//! копируются через IPC.
