//! # UEFI-загрузчик RustOS (этап 0)
//!
//! Последовательность операций (подробности — `docs/BOOT.md`):
//!
...
/// Вспомогательная область после BootInfo (ранние нужды ядра; на этапе 0 не используется).
...
///
/// Поля `scratch`/`stack` не читаются на этапе 0 (вершина стека =
/// `base + size`, scratch используется с этапа 2) — они существуют, чтобы
/// документировать раскладку блока, поэтому `allow(dead_code)`.
#[allow(dead_code)]
struct Layout {
