...
//!
//! Нужны, потому что таргет `x86_64-unknown-rustos` собирается через
...
//!
//! Реализации простые и корректные (побайтовые); оптимизация под SSE/выравнивание —
//! на этапе 2 (память), когда появится реальная нагрузка.
//!
