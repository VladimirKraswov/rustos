...
RIFS initramfs собирает отдельный host tool:

```bash
cargo run -p rustos-pack -- boot/initramfs build/initramfs.img
cargo run -p rustos-pack -- --verify build/initramfs.img
```

Пакетировщик принимает только безопасные относительные UTF-8 пути, проверяет
reserved поля, выравнивание, сортировку и отсутствие пересекающихся data
диапазонов. Новый образ сначала полностью строится и проверяется в памяти,
затем записывается во временный файл рядом с целью и публикуется атомарным
rename; неудачная сборка не повреждает предыдущий initramfs.
