...
pub extern "C" fn _start(_unused: u64, abi_version: u64) -> ! {
    ...
    if !burn_with_extended_state(0xaaaaaaaa_11111111, 0xaaaaaaaa_22222222, 200_000_000) {
        process_exit(112);
    }
    process_exit(21)
}
...
/// Удерживает уникальный 128-битный marker в XMM15 на протяжении busy-loop.
/// Второй процесс использует другой marker: без FXSAVE/FXRSTOR хотя бы один
/// из них детерминированно увидит чужой SIMD context после timer preemption.
