...
test-host:
	cargo test -p rustos-abi -p rustos-pack -p rustos-image -p rustos-gui-check
...
lint:
	cargo fmt --all -- --check
	cargo clippy -p rustos-abi -p rustos-pack -p rustos-image -p rustos-gui-check --all-targets -- -D warnings
