...
test-host:
	cargo test -p rustos-abi -p rustos-microkernel -p rustos-pack -p rustos-image -p rustos-gui-check -p rustos-hmp
...
lint:
	cargo fmt --all -- --check
	cargo clippy -p rustos-abi -p rustos-microkernel -p rustos-pack -p rustos-image -p rustos-gui-check -p rustos-hmp --all-targets -- -D warnings
