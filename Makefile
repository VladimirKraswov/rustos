...
test-host:
	cargo test -p rustos-abi -p rustos-pack -p rustos-image -p rustos-gui-check -p rustos-hmp
...
lint:
	cargo fmt --all -- --check
	cargo clippy -p rustos-abi -p rustos-pack -p rustos-image -p rustos-gui-check -p rustos-hmp --all-targets -- -D warnings
	cargo clippy -Zjson-target-spec -Zbuild-std=core -p rustos-kernel --target targets/x86_64-unknown-rustos.json -- -D warnings
